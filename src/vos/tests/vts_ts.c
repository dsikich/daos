...
run_negative_entry_test(struct ts_test_arg *ts_arg, uint32_t type)
{
	...
	for (parent_idx = 0; parent_idx < ts_arg->ta_counts[type - 1];
		...
		parent = vos_ts_lookup(idx_ptr);
		assert_non_null(parent);
		info = parent->te_info;

		for (idx = 0; idx <= info->ti_cache_mask; idx++) {
			entry = vos_ts_get_negative(parent, idx);
...
run_positive_entry_test(struct ts_test_arg *ts_arg, uint32_t type)
{
	...
	for (idx = 0; idx < ts_arg->ta_counts[type]; idx++) {
		entry = vos_ts_lookup(&ts_arg->ta_records[type][idx]);
		...
		entry = vos_ts_alloc(parent, &ts_arg->ta_records[type][idx],
				     idx);
		/** Entry should be allocated */
		assert_non_null(entry);

		same = vos_ts_lookup(&ts_arg->ta_records[type][idx]);
		/** New lookup should get same entry */
		assert_ptr_equal(same, entry);
	}

	/** Lookup an entry entry */
	entry = vos_ts_lookup(&ts_arg->ta_records[type][NUM_EXTRA - 2]);
	assert_non_null(entry);

	/** Now evict a few entries */
	for (idx = 0; idx < NUM_EXTRA; idx++) {
		entry = vos_ts_alloc(parent, &ts_arg->ta_extra_records[idx],
				     idx);
		assert_non_null(entry);
	}
	...
	same = vos_ts_lookup(&ts_arg->ta_records[type][20]);
	assert_non_null(same);
	vos_ts_evict(&ts_arg->ta_records[type][20]);
	entry = vos_ts_lookup(&ts_arg->ta_records[type][20]);
	assert_null(entry);
	entry = vos_ts_alloc(parent, &ts_arg->ta_records[type][20], 20);
...
ilog_test_ts_get(void **state)
{
	...
	for (type = VOS_TS_TYPE_AKEY;; type -= 2) {
		...
		entry = vos_ts_lookup(&ts_arg->ta_records[type][0]);
		assert_null(entry);
